/**
 * Redcliffe Advisory — Express application.
 *
 * This module only builds and exports the app. Process concerns (listening on a
 * port) live in server.js so the same app can run unchanged as:
 *   - a long-lived Node server (Docker / Kubernetes / `npm start`)
 *   - a Vercel serverless function (api/index.js)
 *   - a Netlify function (netlify/functions/server.js)
 */
...
const { PUBLIC_FILES } = require('./site-manifest');
...
// On a developer machine we still default to the local Docker Postgres. On a
// serverless platform there is no localhost database, so an unset DATABASE_URL
// means "run the site without the CMS" rather than "hang on a dead socket".
const LOCAL_DATABASE_URL = 'postgres://radcliffe:radcliffe@localhost:5432/radcliffe';
const DATABASE_URL = process.env.DATABASE_URL || (IS_SERVERLESS ? '' : LOCAL_DATABASE_URL);
const CMS_ENABLED = Boolean(DATABASE_URL);

const ADMIN_USERNAME = process.env.ADMIN_USERNAME || 'admin';
// Never fall back to a shipped default password/secret in production — that
// would leave the CMS on a public domain open with well-known credentials.
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || (IS_PRODUCTION ? '' : 'radcliffe');
const SESSION_SECRET = process.env.SESSION_SECRET || (IS_PRODUCTION ? '' : 'dev-only-session-secret');
const ADMIN_ENABLED = CMS_ENABLED && Boolean(ADMIN_PASSWORD) && Boolean(SESSION_SECRET);
...
function resolveSsl(connectionString) {
  ...
  // Managed Postgres (Neon, Supabase, Vercel Postgres, RDS…) requires TLS;
  // an in-cluster or local database usually does not offer it.
  if (/@(localhost|127\.0\.0\.1|radcliffe-db|postgres)[:/]/i.test(connectionString)) return false;
  return { rejectUnauthorized: false };
}

let pool;
function getPool() {
  if (!CMS_ENABLED) throw new Error('DATABASE_URL is not configured');
  if (!pool) {
    pool = new Pool({
      connectionString: DATABASE_URL,
      ssl: resolveSsl(DATABASE_URL),
      // A serverless invocation handles one request, so one connection is
      // plenty and keeps the database's connection count sane under fan-out.
      max: IS_SERVERLESS ? 1 : 10,
      idleTimeoutMillis: IS_SERVERLESS ? 5000 : 30000,
...
/**
 * Migrations are idempotent but chatty, so a warm/seeded database short-circuits
 * after a single cheap query. That matters on serverless, where this runs once
 * per cold start rather than once per deploy.
 */
async function migrate() {
  if (process.env.SKIP_DB_MIGRATION === 'true') return;
  if (process.env.FORCE_DB_MIGRATION !== 'true') {
    const existing = await query("SELECT to_regclass('public.cms_sections') AS table_name");
    if (existing.rows[0].table_name) {
      const seeded = await query('SELECT count(*)::int AS count FROM cms_sections');
      if (seeded.rows[0].count >= sectionSeeds.length) return;
    }
  }
  await createSchema();
}
...
function ensureSchema() {
  if (!schemaReady) {
    schemaReady = migrate().catch((error) => {
      schemaReady = null; // let the next request retry instead of caching failure
      throw error;
    });
  }
  return schemaReady;
...
// this an async route failure leaves the request hanging until it times out.
...
function requireAdmin(req, res, next) {
  if (!ADMIN_ENABLED) return respondCmsDisabled(req, res);
  try {
...
function missingAdminConfig() {
  const missing = [];
  if (!CMS_ENABLED) missing.push('DATABASE_URL');
  if (!ADMIN_PASSWORD) missing.push('ADMIN_PASSWORD');
  if (!SESSION_SECRET) missing.push('SESSION_SECRET');
  return missing;
}

function respondCmsDisabled(req, res) {
  const missing = missingAdminConfig();
  if (req.path.startsWith('/api/')) {
    return res.status(503).json({ error: 'CMS is not configured', missing });
  }
  return res.status(503).type('html').send(`<!doctype html>
<meta charset="utf-8" />
<title>CMS not configured — Redcliffe Advisory</title>
<style>
  body { font: 16px/1.6 system-ui, sans-serif; margin: 0; display: grid; place-items: center;
         min-height: 100vh; background: #0C2138; color: #F6F8FB; padding: 2rem; }
  main { max-width: 34rem; } code { background: rgba(255,255,255,.1); padding: .1rem .35rem; border-radius: 3px; }
  a { color: #8FBEE2; }
</style>
<main>
  <h1>The CMS is not configured</h1>
  <p>The public website is live, but the admin portal needs these environment
     variables set on the hosting platform:</p>
  <p><code>${missing.join('</code> <code>')}</code></p>
  <p>See <code>docs/DEPLOYMENT.md</code> in the repository for the exact steps.</p>
...
async function getContentValue(key, fallback = '') {
  const result = await query('SELECT value FROM cms_content WHERE key = $1', [key]);
...
// Lets a deployment be checked before DNS is switched over.
app.get('/api/health', (_req, res) => {
  res.json({
    ok: true,
    cms: CMS_ENABLED ? 'configured' : 'disabled',
    admin: ADMIN_ENABLED ? 'enabled' : 'disabled',
    contactEmail: process.env.SMTP_HOST ? 'smtp' : CMS_ENABLED ? 'stored' : 'disabled',
  });
});

app.get('/api/content', async (_req, res) => {
  if (!CMS_ENABLED) {
    // The pages ship with their copy inline, so cms-client.js simply keeps the
    // built-in content when this responds 503.
    return res.status(503).json({ error: 'CMS content is unavailable' });
  }
  try {
    await ensureSchema();
...
app.get(
  ...
  asyncRoute(async (req, res) => {
    if (!CMS_ENABLED) return res.sendStatus(404);
    await ensureSchema();
    const result = await query('SELECT mime_type, data, updated_at FROM cms_media WHERE key = $1', [
      req.params.key,
    ]);
...
app.post(
  '/api/contact',
  asyncRoute(async (req, res) => {
    if (!CMS_ENABLED && !process.env.SMTP_HOST) {
    ...
    const fallbackRecipient =
      process.env.CONTACT_TO_EMAIL || 'karina.robinson@redcliffeadvisory.com';
    let recipientEmail = fallbackRecipient;
    if (CMS_ENABLED) {
      await ensureSchema();
      recipientEmail = await getContentValue('contact.recipient.email', fallbackRecipient);
    }
    ...
    if (CMS_ENABLED) {
      ...
      // No database to fall back on and the email did not go out.
      return res.status(502).json({ error: 'Unable to send message. Please email us directly.' });
    }
...
app.delete(
  ...
  asyncRoute(async (req, res) => {
    await ensureSchema();
    const existing = await query('SELECT is_static FROM cms_sections WHERE key = $1', [req.params.key]);
    ...
    await query('DELETE FROM cms_sections WHERE key = $1', [req.params.key]);
    return res.json({ deleted: true, key: req.params.key });
  })
);

/* ----------------------------------------------------------- static website */
/* On Vercel/Netlify these routes are never reached: the CDN serves dist/ and
   only /api/* and /admin* are routed to this app. They keep `npm start` and the
   Docker/Kubernetes deployment serving the whole site from one process. */
...
app.get('*', (req, res, next) => {
  ...
  if (!fs.existsSync(homepage)) return next();
...
module.exports.CMS_ENABLED = CMS_ENABLED;
